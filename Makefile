...
build: clean fmt
	GOARCH=$(ARCH) CGO_ENABLED=0 go build -ldflags "$(HEAPSTER_LDFLAGS)" -o $(OUTPUT_DIR)/heapster k8s.io/heapster/metrics
	GOARCH=$(ARCH) CGO_ENABLED=0 go build -ldflags "$(HEAPSTER_LDFLAGS)" -o $(OUTPUT_DIR)/eventer k8s.io/heapster/events
...
build-dockerfile:
	echo REPO_DIR=$(REPO_DIR) OUTPUT_DIR=$(OUTPUT_DIR) Dockerfile=$(REPO_DIR)/deploy/docker/Dockerfile
	cp $(REPO_DIR)/deploy/docker/Dockerfile $(OUTPUT_DIR)
	docker build --pull -t $(PREFIX)/heapster-$(ARCH):$(VERSION) $(OUTPUT_DIR)
...
sub-push-%:
	$(MAKE) ARCH=$* PREFIX=$(PREFIX) VERSION=$(VERSION) run-container
	$(MAKE) ARCH=$* PREFIX=$(PREFIX) VERSION=$(VERSION) build-dockerfile
